...
func InitGUI() {
	...
	// network device
	selectNetDevice := widget.NewSelect(service.GetNetworkInterfaces(), func(selected string) {})
	...
	buttonTunnelServiceStat := widget.NewButton("Start / Stop", func() {
		...
		if !checkPcap(window) {
		} else if !checkIPAddress(selecteddstIP, window) {
			...
			go service.CapturePackets(stopThreadChannel, selectNetDevice.Selected, selecteddstIP, port)
			serviceRunning = true
		} else {
...
func checkPcap(window fyne.Window) bool {
	if service.IsPcapSetupCorrect() == false {
