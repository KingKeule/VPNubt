...
func InitGUI() {
	...
	app.Settings().SetTheme(&ThemeCustom{"Grey"})
	...
	widgetGroupPing := container.New(layout.NewGridLayout(2), buttonPing, widgetPingStatus)
	...
	widgetGroupStatus := container.New(layout.NewGridLayout(2),
	...
	bgLayerRed := canvas.NewRectangle(color.NRGBA{R: 255, G: 0, B: 0, A: 255})
	bgLayerGreen := canvas.NewRectangle(color.NRGBA{R: 0, G: 255, B: 0, A: 255})
	...
	containerServiceCmd := container.New(layout.NewStackLayout(), container.New(layout.NewPaddedLayout(), bgLayerRed, bgLayerGreen), buttonTunnelServiceStat)

	// ---------------- space area ----------------
	spaceArea := canvas.NewRectangle(fyne.CurrentApp().Settings().Theme().Color(theme.ColorNameBackground, fyne.CurrentApp().Settings().ThemeVariant()))
	spaceArea.SetMinSize((fyne.NewSize(0, 10)))

	// ---------------- Container complete ----------------
	containerAll := container.NewVBox(
	...
	window.SetMainMenu(fyne.NewMainMenu(
		fyne.NewMenu("Tool",
			fyne.NewMenuItem("Reset all values", func() {
				if serviceRunning {
					servRunWarnText1 := "Service is running."
					servRunWarnText2 := "Stop before reset!"
					log.Println("Error: " + servRunWarnText1 + " " + servRunWarnText2)
					errServRun := widget.NewLabel(servRunWarnText1 + "\n" + servRunWarnText2 + "\n")
					customDialog := dialog.NewCustom("Error", "OK", errServRun, window)
					customDialog.Resize(fyne.NewSize(window.Canvas().Size().Width-40, window.Content().Size().Height/3))
					customDialog.Show()
				} else {
		...
		fyne.NewMenu("Help",
			...
			fyne.NewMenuItem("About", func() {
				versInfo := widget.NewLabel("Version: " + version + "\n")
				customDialog := dialog.NewCustom("About", "OK", versInfo, window)
				customDialog.Resize(fyne.NewSize(window.Canvas().Size().Width-40, window.Content().Size().Height/3))
				customDialog.Show()
			}),
		)))
...
func updateGui(widgetPktCntValue *widget.Label, pktCntChannel chan bool) {
	for pktCntChannelValue := range pktCntChannel {
		fyne.Do(func() {
			if pktCntChannelValue {
				fwdPktCnt++
...
func checkIPAddress(ip net.IP, window fyne.Window) bool {
	if (ip == nil) || (ip.IsUnspecified()) {
		ipWarnText1 := "The IP address is not correct."
		ipWarnText2 := "Please set a valid IP address."
		log.Println("Error: " + ipWarnText1 + " " + ipWarnText2)
...
func checkPort(err error, port int, window fyne.Window) bool {
	if (err != nil) || (port < 1) || (port > 65535) {
		portWarnText1 := "The UDP port is not correct."
		portWarnText2 := "Please set a valid UDP port (1-65535)."
		log.Println("Error: " + portWarnText1 + " " + portWarnText2)
...
func writeConfigToFile(inputdstIP string, inputsrcPort int, window fyne.Window) {
	pwd, _ := os.Getwd()
	log.Printf("Try to write the actual IP address and UDP port to a configuration file \"%s\" in the same directory from which the program was launched \"%s\"", configFileName, pwd)
	...
	if err != nil {
		...
		log.Printf("Configuration file was saved successfully to \"%s\\%s\"", pwd, configFileName)
		dialog.ShowInformation("", "Configuration file was saved successfully to: \""+pwd+"\\"+configFileName+"\"", window)
	}
}

func checkForConfig() *config.Config {
	pwd, _ := os.Getwd()
	log.Printf("Try to read the configuration file \"%s\" from the same directory from which the program was launched \"%s\"", configFileName, pwd)

	configFileBytes, err := os.ReadFile(configFileName)
	if err != nil {
		log.Printf("No configuration file was found in the same directory. Using no config")
		return config.GetDefaultConf()
	}

	log.Printf("Configuration file was found in the same directory and will be imported")
	var actConfig config.Config
	err = json.Unmarshal([]byte(configFileBytes), &actConfig)
	if err != nil {
		log.Printf("Error while marshalling configuration file. Error: %s", err)
		return config.GetDefaultConf()
	} else {
		return &actConfig
	}
